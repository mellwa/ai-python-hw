...
class InputOutput:
    ...
    def write_file(self):
        ...
        output_file.write(self.__output_info.get_next_move()+'\n')
...
class Solution:
    ...
    def print_tree(self):
        ...
        self.print_path(self.__state, output_file)

    counter = 0

    def print_path(self, node, output_file, depth=0):
        if node is not None:
            if node.previous_op() is not None:
                self.counter += 1
                msg = str(self.counter)+": depth"+str(depth)+":"+str(node.previous_op())
                output_file.write(msg+'\n')
            for n in node.neighbours():
                self.print_path(n, output_file, depth+1)
